...
// HTTP 拉取 /json/version 获取真实的 webSocketDebuggerUrl + Browser/UserAgent 标识
//   原因：DevToolsActivePort 文件可能是过期的（用户主 Chrome 上次启动时写的 UUID，
//         即使现在 9222 端口被另一个 Chrome 实例占用，文件 UUID 也不会自动更新）
//   策略：永远以 /json/version 返回的实时 wsPath 为准
//   SAFETY: 同时返回 Browser + User-Agent，调用方据此拒绝非 HeadlessChrome 实例
//     注意：Chrome --headless=new 在 Browser 字段写 "Chrome/..."（无 Headless 前缀），
//     但在 User-Agent 里有 "HeadlessChrome/..."。所以必须两个字段都检查。
function fetchVersionInfo(port) {
    ...
    const req = http.get({ host: '127.0.0.1', port, path: '/json/version', timeout: 2000 }, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          resolve({
            wsPath: m ? m[1] : null,
            browser: typeof json.Browser === 'string' ? json.Browser : '',
            userAgent: typeof json['User-Agent'] === 'string' ? json['User-Agent'] : '',
          });
        } catch { resolve({ wsPath: null, browser: '', userAgent: '' }); }
      });
    });
    req.on('error', () => resolve({ wsPath: null, browser: '', userAgent: '' }));
    req.on('timeout', () => { req.destroy(); resolve({ wsPath: null, browser: '', userAgent: '' }); });
  });
}

// Backward-compat shim — older call sites only need wsPath
function fetchVersionWsPath(port) {
  return fetchVersionInfo(port).then(info => info.wsPath);
}

// --- 自动发现 Chrome 调试端口 ---
async function discoverChromePort() {
  // 1. 尝试读 DevToolsActivePort 文件（仅取端口号，wsPath 必须用 /json/version 校准）
  const possiblePaths = [];
  ...
  if (platform === 'darwin') {
    ...
    possiblePaths.push(
      // 优先专属 CDP profile（check-deps.sh 启动的独立实例）
      path.join(home, '.cache/chrome-cdp-profile/DevToolsActivePort'),
  ...
  // SAFETY: if CDP_CHROME_PORT is set, ONLY probe that port. This is the pin
  // installed by check-deps.sh — guarantees we never accidentally discover the
  // user's main Chrome on a different port.
  const pinned = parseInt(process.env.CDP_CHROME_PORT || '0');
  if (pinned > 0 && pinned < 65536) {
    candidatePorts.add(pinned);
  } else {
    for (const p of possiblePaths) {
      try {
        const content = fs.readFileSync(p, 'utf-8').trim();
        const port = parseInt(content.split('\n')[0]);
        if (port > 0 && port < 65536) candidatePorts.add(port);
      } catch { /* 文件不存在 */ }
    }
    // 兜底端口：默认只用 9333（专属 headless 端口），避免落在用户主 Chrome 的 9222
    // 旧的 [9222, 9229, 9333] 顺序会让本 proxy 优先连上用户主 Chrome
    for (const p of [9333]) candidatePorts.add(p);
  }

  // SAFETY: only attach to HeadlessChrome instances (unless explicitly opted out)
  const allowUserChrome = process.env.CDP_ALLOW_USER_CHROME === '1';

  // 逐个候选端口探测，用 /json/version 拿到的实时 wsPath（避免使用过期 UUID）
  for (const port of candidatePorts) {
    if (!await checkPort(port)) continue;
    const info = await fetchVersionInfo(port);
    if (info.wsPath) {
      // SAFETY: 接受标准是 Browser 或 User-Agent 任一含 "HeadlessChrome"
      //   Chrome --headless=new 在 Browser 字段写 "Chrome/..."（无前缀），
      //   只在 User-Agent 里有 "HeadlessChrome/..."。
      const isHeadless = /HeadlessChrome/i.test(info.browser) || /HeadlessChrome/i.test(info.userAgent);
      if (!allowUserChrome && !isHeadless) {
        console.log(`[CDP Proxy] 跳过端口 ${port}：非 HeadlessChrome 实例 (Browser=${info.browser}, UA=${info.userAgent.substring(0, 60)}). 设置 CDP_ALLOW_USER_CHROME=1 以覆盖。`);
        continue;
      }
      console.log(`[CDP Proxy] 发现 Chrome: 端口 ${port}, wsPath ${info.wsPath} (headless=${isHeadless})`);
      return { port, wsPath: info.wsPath };
    }
    // /json/version 不可达但端口在听 → 可能是用户主 Chrome（不暴露 /json/version）
    // SAFETY: 拒绝连接，避免误控用户浏览器
    if (!allowUserChrome) {
      console.log(`[CDP Proxy] 跳过端口 ${port}：/json/version 不可达，无法验证身份`);
      continue;
    }
    console.log(`[CDP Proxy] 端口 ${port} 在听但 /json/version 失败，使用默认 wsPath (CDP_ALLOW_USER_CHROME=1)`);
